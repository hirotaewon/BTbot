	...
	if client.get_channel(channel) != msg.channel :
		...
		##################################
			...
			if message.content.startswith(bossData[i][0] +'예약'):
				...
				if hello.find(' ') != -1 :
					chkpos = hello.find(' ')
					hours1 = hello[chkpos+1:chkpos+3]
					minutes1 = hello[chkpos+3:chkpos+6]
					now2 = datetime.datetime.now()
					tmp_now = datetime.datetime.now()
					tmp_now = tmp_now.replace(hour=int(hours1), minute=int(minutes1))
				...
				if now2 > tmp_now :
					tmp_now = tmp_now + datetime.timedelta(days=1) 

				bossTime[i] = nextTime = tmp_now
				tmp_bossTimeString[i] = bossTimeString[i] = nextTime.strftime('%H:%M:%S')
				tmp_bossDateString[i] = bossDateString[i] = nextTime.strftime('%Y-%m-%d')
				embed = discord.Embed(
						description= '다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.',
		...
		if message.content.startswith('!계산'):
			...
			mathLen = message.content.split(' ')[1:]
			tax = math.ceil(int(mathLen[0]) * 0.05)
			distribution = int(mathLen[1]) - tax
			payback  = int(mathLen[0]) - int(mathLen[1])
			paybackTax = math.ceil((payback) * 0.05) + int(mathLen[1])
			result = '분배금: '+repr(distribution)+'\n세금: '+repr(tax) + '\n페이백: '+repr(payback) + '\n구매자부담: ' + repr(paybackTax)
			print("==== caculate End ====")
